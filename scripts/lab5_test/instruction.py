...
class ImulInstruction(Instruction):
    ...
    def execute(self, state_table):
        ...
        # Get value of second operand, it must be register, and store result in it
        if re.match(OperandEnum.Reg.value, self._arg1):
            arg1_val = state_table.load_reg(self._arg1)
            result = arg0_val * arg1_val
            mark_bit = ~(1 << 63)
            sign_bit = 1 if result < 0 else 0
            rax_res = (result & mark_bit) | (sign_bit << 63)
            state_table.store_reg(self._arg1, rax_res)
        elif re.match(OperandEnum.Imm.value, self._arg1):
            raise AssertionError("Must use Register as second operand")
...
class JmpInstruction(Instruction):
    ...
    def print_kind(self):
        print("Jmp Instruction "+self._label)
        
class SetInstruction(Instruction):
        
        # TODO(lz): Comment
    
        def __init__(self, kind, condition, arg0):
            super(SetInstruction, self).__init__(kind)
            self._arg0 = arg0
            self._condition = condition
    
        def execute(self, state_table):
            # Validate the condition
            if state_table.validate_set_condition(self._condition):
                state_table.store_reg(self._arg0, 1)
            else:
                state_table.store_reg(self._arg0, 0)

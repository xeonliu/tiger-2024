import re
from type import OperandEnum,JumpEnum,SetEnum
import sys
...
class StateTable(object):
    ...
    def _construct_string(self, string_table):
        for index, label in enumerate(string_table.keys()):
            # index = int(label[1:])
            ...
            self._label_table[label] = string_address
    ...
    def get_operand(self, operand):
        ...
        elif operand.find('framesize') != -1 and operand.find('$') != -1:
            for frame_name in self._frame_size.keys():
                if operand.find(frame_name) != -1:
                    operand_val = int(self._frame_size[frame_name])
                    break
    ...
    def validate_set_condition(self, condition):
        arg0_val = self._copmarison[0]
        arg1_val = self._copmarison[1]
        if condition == SetEnum.E:
            return arg1_val == arg0_val
        elif condition == SetEnum.Ne:
            return arg1_val != arg0_val
        elif condition == SetEnum.G:
            return arg1_val > arg0_val
        elif condition == SetEnum.Ge:
            return arg1_val >= arg0_val
        elif condition == SetEnum.L:
            return arg1_val < arg0_val
        elif condition == SetEnum.Le:
            return arg1_val <= arg0_val
        else:
            AssertionError("Not a valid jump instruction")
    ...
    def get_mem_address(self, mem):
        ...
        imm = 0
        if imm_index != -1 and mem.find('(%rip)') == -1:
            if imm_index == 0:
                imm = 0
            ...
            if mem.find('(%rip)') != -1:
                name=mem.split('(%rip)')[0]
                address = self._label_table[name]
                # Relative mem_address
                return address
            else:
                return self._string_address + int(mem[1:]) * 8
